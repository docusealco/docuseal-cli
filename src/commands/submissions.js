...
export function registerSubmissionCommands(program) {
  ...
  topic
    ...
    .option('--archived', 'Returns only archived submissions when `true` and only active submissions when `false`.')
    .option('--no-archived', '')
  ...
  topic
    ...
    .option('--send-email', 'Set `false` to disable signature request emails sending.')
    .option('--no-send-email', '')
    .option('--send-sms', 'Set `true` to send signature request via phone number and SMS.')
    .option('--no-send-sms', '')
  ...
  topic
    .command('create-pdf')
    .description('Create a submission from PDF')
    .option('--api-key <value>', 'Override API key for this invocation')
    .option('--server <value>', 'Server: com, eu, or full URL')
    .option('-d, --data <value>', 'Set body parameters using bracket notation (e.g. -d "submitters[0][email]=john@acme.com")', (val, prev) => prev.concat([val]), [])
    ...
    .option('--no-flatten', '')
    .option('--merge-documents', 'Set `true` to merge the documents into a single PDF file.')
    .option('--no-merge-documents', '')
    .option('--remove-tags', 'Pass `false` to disable the removal of {{text}} tags from the PDF.')
  ...
  topic
    .command('create-docx')
    .description('Create a submission from DOCX')
    .option('--api-key <value>', 'Override API key for this invocation')
    .option('--server <value>', 'Server: com, eu, or full URL')
    .option('-d, --data <value>', 'Set body parameters using bracket notation (e.g. -d "submitters[0][email]=john@acme.com")', (val, prev) => prev.concat([val]), [])
    .addOption(new Option('--name <value>', 'Name of the document submission.'))
  ...
  topic
    .command('create-html')
    .description('Create a submission from HTML')
    .option('--api-key <value>', 'Override API key for this invocation')
    .option('--server <value>', 'Server: com, eu, or full URL')
    .option('-d, --data <value>', 'Set body parameters using bracket notation (e.g. -d "submitters[0][email]=john@acme.com")', (val, prev) => prev.concat([val]), [])
    .addOption(new Option('--name <value>', 'Name of the document submission'))
